// src/actions/rebalanceWallet.ts
import {
  ModelClass,
  elizaLogger,
  composeContext,
...
import { settings as settings2 } from "@elizaos/core";
import { privateKeyToAccount } from "viem/accounts";
var useGetAccount = () => {
  let PRIVATE_KEY = settings2.PRIVATE_KEY_EXECUTOR;
  if (!PRIVATE_KEY) {
...
var rebalanceWallet = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    try {
      const { recipient: recipient2, amount, sourceNetwork: sourceNetwork2, destinationNetwork: destinationNetwork2, fromAsset: fromAsset2, toAsset: toAsset2 } = result.data;
      ...
      if (callback) {
        callback({
          ...
          content: {
            ...
            sourceNetwork: sourceNetwork2,
            destinationNetwork: destinationNetwork2
          }
        });
      }
      return true;
...
// src/actions/analyzeConditions.ts
import {
...
import { getExecutor as getExecutor2 } from "@t3rn/executor";

// src/utils/helpers.ts
import { BigNumber } from "@ethersproject/bignumber";
var convertBigNumbersToString = (obj) => {
...
var generateAnalyzeTemplate = (data) => {
  return `
...
var analyzeConditions = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger2.info("Starting START_ANALYSIS handler...");
    let currentState = state;
    if (!currentState) {
      currentState = await runtime.composeState(message);
    } else {
    ...
    const executorAddress = account.address;
    try {
      const executor = await getExecutor2();
      ...
      const enabledArbitrageStrategy = Object.fromEntries(
        Object.entries(arbitrageStrategy).filter(
          ([networkId]) => enabledNetworksIds.includes(networkId)
        )
      );
      ...
      const analyzeContext = composeContext2({
        state: currentState,
      ...
      elizaLogger2.error({ executor: executorAddress, error: error.message }, "Error analyzing conditions");
      if (callback) {
        callback({
...
// src/actions/rebalanceStrategy.ts
import {
...
import { getExecutor as getExecutor3 } from "@t3rn/executor";
import { formatEther } from "viem";
var OrderArbitrageStrategySchema = z.object({
...
var ArbitrageStrategyRebalanceSchema = z.record(
  z.record(OrderArbitrageStrategySchema)
);
var validatedSchema2 = z.object({
  strategies: ArbitrageStrategyRebalanceSchema
});
var rebalanceStrategyTemplate = `Respond ONLY with a JSON markdown block containing the extracted values. Do not output any extra text. The JSON must match EXACTLY the structure shown in the example.
...
var rebalanceStrategy = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger3.info("Starting REBALANCE_STRATEGY handler...");
    let currentState = state;
    if (!currentState) {
      currentState = await runtime.composeState(message);
    } else {
    ...
    currentState.currentMessage = message.content.text;
    try {
      const executor = await getExecutor3();
      ...
      const content = (await generateObject3({
        ...
        schema: validatedSchema2
      })).object;
      const input = {
        strategies: content.strategies
      };
      const result = validatedSchema2.safeParse(input);
      if (!result.success) {
        elizaLogger3.error(
          { executor: executorAddress, input, error: result.error.message },
      ...
      for (const network in strategies) {
        ...
        for (const asset in strategies[network]) {
          ...
          newStrategies[network][asset] = {
            minProfitPerOrder: newStrategy.minProfitPerOrder !== void 0 ? newStrategy.minProfitPerOrder : currentStrategy.minProfitPerOrder,
            minProfitRate: newStrategy.minProfitRate !== void 0 ? newStrategy.minProfitRate : currentStrategy.minProfitRate,
            maxAmountPerOrder: newStrategy.maxAmountPerOrder !== void 0 ? newStrategy.maxAmountPerOrder : currentStrategy.maxAmountPerOrder,
            minAmountPerOrder: newStrategy.minAmountPerOrder !== void 0 ? newStrategy.minAmountPerOrder : currentStrategy.minAmountPerOrder,
            maxShareOfMyBalancePerOrder: newStrategy.maxShareOfMyBalancePerOrder !== void 0 ? newStrategy.maxShareOfMyBalancePerOrder : currentStrategy.maxShareOfMyBalancePerOrder
          };
        }
      }
      ...
      elizaLogger3.error({ error: error.message }, "Error during arbitrage strategy rebalance");
      if (callback) {
        callback({
...
function sleep(seconds, reason) {
  elizaLogger4.trace({ reason }, `Sleeping for ${seconds} sec...`);
...
// src/services/analysis.ts
import { getExecutor as getExecutor4, startExecutor } from "@t3rn/executor";
var executorAnalysis = {
...
var startExecutorLib = async () => {
  elizaLogger5.info("Starting executor");
  const executor = await getExecutor4();
  const executorId = executor.getMyAddress();
  const account = useGetAccount();
  if (executorId !== account.address) {
    elizaLogger5.error({ executorId, account: account.address }, "Executor address does not match account address");
    process.exit(1);
  }
  await startExecutor();
};
var startAIExecutor = async (runtime) => {
  elizaLogger5.info("Starting AI Executor");
  let sleepSec = 60;
  ...
  while (true) {
    try {
      ...
      if (!analysisAction) {
        elizaLogger5.error("No analysis action found!");
        return;
      }
      if (!rebalanceWalletAction) {
        elizaLogger5.error("No rebalance wallet action found!");
        return;
      }
      if (!rebalanceStrategyAction) {
        elizaLogger5.error("No rebalance strategy action found!");
        return;
      }
      const handlerCallback = async (response) => {
        elizaLogger5.info({ content: response.content ?? response }, `Agent: ${response.text?.trim()}`);
        const newMemory2 = {
        ...
        await runtime.messageManager.createMemory(newMemory2);
        return [newMemory2];
      };
      const result = await analysisAction.handler(runtime, message, currentState, {}, handlerCallback);
      if (!result) {
        elizaLogger5.error("No result found from analysis action");
        return;
      }
      ...
      if (!shouldRebalanceWallet && !shouldRebalanceStrategy) {
        elizaLogger5.info({ walletReasoning, strategyReasoning, sleepSec }, "Agent considers there's no need to rebalance. Sleep for now.");
        await sleep(sleepSec);
        continue;
      }
      if (shouldRebalanceWallet) {
        const newMemoryRebalance = {
      ...
      elizaLogger5.info({ sleepSec }, "Executor analysis completed");
    } catch (error) {
      elizaLogger5.error({ error: error.message, sleepSec }, "Error in executor analysis:");
    }
    await sleep(sleepSec);
  }
};
